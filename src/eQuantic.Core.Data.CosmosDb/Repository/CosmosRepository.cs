...
/// <summary>
///     The native Azure Cosmos DB repository — the read surface from <see cref="CosmosReadRepository{TEntity, TKey}" />
///     plus writes. Entity writes (<c>Add</c>/<c>Modify</c>/<c>Remove</c>/<c>Merge</c>) are staged on the unit of
///     work and flushed on commit; set-based writes (<c>DeleteMany</c>/<c>UpdateMany</c>) query the matching items
///     and then delete or patch each one immediately.
/// </summary>
/// <typeparam name="TEntity">The entity type.</typeparam>
/// <typeparam name="TKey">The key type.</typeparam>
public class CosmosRepository<TEntity, TKey> :
    CosmosReadRepository<TEntity, TKey>,
    ...
    where TEntity : class, IEntity<TKey>
{
    ...
    public CosmosRepository(IQueryableUnitOfWork unitOfWork) : base(unitOfWork)
    {
    }

    private CosmosSet<TEntity> Set() => new(UnitOfWork, Container);

    private string IdOf(TEntity item) => UnitOfWork.Configuration<TEntity>().GetId(item);
    ...
    /// <inheritdoc />
    public void Remove(TEntity item) => UnitOfWork.StageDelete(item, IdOf(item));
    ...
    {
        UnitOfWork.StageDelete(item, IdOf(item));
        return Task.CompletedTask;
    }
